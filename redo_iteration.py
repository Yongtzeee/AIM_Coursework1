...
for round in range(10): #-
  ...
  def do_diff_chrom_v2(mat_chrom_accur, num_new_sol, num_chrom_params):
    ...
    # Scan through new solutions
    for si in range(num_new_sol):
      random_neighbour_gen = np.divide(np.random.rand(num_chrom_params), difference_of_chroms)
  ...
  for shci in range(meta['max_shc_iter']):
    ...
    has_improved = False #-
    countdown_iter = 3  #-
    while not has_improved and countdown_iter > 0:  #-    # either try again until has improved or up to n times

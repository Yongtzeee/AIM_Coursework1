...
def local_search(a_chrom, num_chrom_params):
  global best_res, local_search_iter
  # local search:
...
for ti in range(trials):
  ...
  mat_chroms = [[0 for x in range(comp_num_chrom_param(limits)+1)] for y in range(meta['max_rs_iter']*2)]
  ...
  for rsi in range(meta['max_rs_iter']*2):
  ...
  # ### --- pseudocode for population based search algorithm
  # 1.  initialize random population and save current chromosome population as destination
  # 2.  generate step, differential_iteration
  # 3.  displace chromosomes and evaluate them, save displaced chromosomes as start_point
  # 4.  while stop criteria has not reached
  # 5.    for each chromosome in population, do
  # 6.      if chromosome has not found a better solution and have not reached to destination, then
  # 7.        keep heading towards destination one step at a time, evaluate chromosome at each step
  # 8.      else if chromosome has found a better solution, then
  # 9.        perform local search to find best solution in the area
  # 10.       set best solution as new destination
  # 11.       displace chromosome
  # 12.     else if chromosome has reached destination, then
  # 13.       set start_point as new destination and displace chromosome
  # 14.     end if
  # 15.   end for
  # 16.   if differential_iteration == t, then  (t is positive integer)
  # 17.     perform differential search on population
  # 18.     replace worst performing chromosomes with better ones obtained from differential search
  # 19.   end if
  # 20.   differential_iteration += 1
  # 21. end while
  # 22. perform final test on best chromosome and obtain result

  # ### --- pseudocode for local search algorithm
  # 1.  start from a chromosome
  # 2.  randomly generate a directed vector to determine direction for search
  # 3.  add directed vector to chromosome and evaluate the newly generated chromosome
  # 4.  if new chromosome is better, then
  # 5.    

  # ### --- pseudocode for differential search algorithm
  # 1.  

  terminate_search = False
  pop_search_iter = 0

  destination_chroms = mat_chroms[0:math.floor(len(mat_chroms)/2)]
  start_chroms = mat_chroms[math.floor(len(mat_chroms)/2):]
  chroms_list = start_chroms
  
  step_size = 0.3 # 0.4 # 0.2 # 0.1
  # step_denom = 3 # 4 # 5
  # radius = 12

  # step tracker for each chromosome
  steps_before_displace = math.floor(1/step_size)
  displace_when_0 = [steps_before_displace for i in range(len(start_chroms))]

  vector_chroms = np.subtract(destination_chroms, start_chroms)
  vector_chroms = np.array([i[1:] for i in vector_chroms])
  vector_chroms = vector_chroms * step_size
  while not terminate_search:
    ...
    new_chromosomes = []
    replace_index = []
    # step size static -> step_size
    # step size dynamic -> step_denom
    # greedy approach (perform local search upon first encounter of better result)
    # hold off and see if there is better result

    # step size static + greedy approach
    for i, a_chrom in enumerate(chroms_list):
      if a_chrom[0] > destination_chroms[i][0]:
        # if current chromosome's result is better than the destination's, perform local search
        # change the destination to the current chromosome and displace the current chromosome

        # best_chrom = local_search(a_chrom, num_chrom_params)
        destination_chroms[i] = a_chrom # best_chrom
        # displace the chromosome to a random place in the search space
        new_chrom = gen_rand_chromosome(num_chrom_param)
        # index tracker for tracking the chromosomes to replace as the starting point
        replace_index.append(i)
        # reset displacement counter
        displace_when_0[i] = steps_before_displace
      elif displace_when_0[i] <= 0:
        # if current chromosome has reached destination, set the destination to either 
        # the better performing destination of the neighbouring chromosome or 
        # current chromosome's starting point and displace current chromosome
        
        next_destination = sorted(destination_chroms, reverse=True)[0]
        # next_destination = start_chroms[i]
        # min_dis = 1000
        # for j, chrom in enumerate(destination_chroms):
        #   if i == j:
        #     continue
        #   if chrom[0] > a_chrom[0]:
        #     # n-dimensional Pythagorean theorem to calculate Euclidean distance between the chromosomes
        #     sum_params = 0.0
        #     for k in np.subtract(chrom[1:], a_chrom[1:]):
        #       sum_params += k**2
        #     dis = math.sqrt(sum_params)
        #     # # check if the destination is near enough to our chromosome and whether it's the nearest
        #     # if dis <= radius and dis < min_dis:
        #     #   min_dis = dis
        #     #   next_destination = destination_chroms[j]
        #     # check if the destination is the nearest to our chromosome
        #     if dis < min_dis:
        #       min_dis = dis
        #       next_destination = destination_chroms[j]
        
        # set next destination point for our chromosome
        destination_chroms[i] = next_destination
        # displace the chromosome to a random place in the search space
        new_chrom = gen_rand_chromosome(num_chrom_param)
        # index tracker for tracking the chromosomes to replace as the starting point
        replace_index.append(i)
        ...
        # if current chromosome's result is not better nor has it reached its destination,
        # add a step to the chromosome to continue heading towards destination

        # add a step
        new_chrom = np.add(a_chrom[1:], vector_chroms[i])
        displace_when_0[i] -= 1
      
      np.clip(new_chrom, 0, 0.99999999999, out=new_chrom)
      new_chromosomes.append(new_chrom)
    
    next_res, mat_chrom_accur = eval_chromosomes(new_chromosomes, num_chrom_params)
    if next_res[0] > best_res[0]:
      best_res = next_res
    for i in replace_index:
      start_chroms[i] = mat_chrom_accur[i]
    chroms_list = mat_chrom_accur.tolist()

    # ### --- differential search

    # # # --- Gravitational Search Algorithm
    # new_chroms = gravitational_search(mat_chroms, num_chrom_params)
    # mat_chroms = sorted(new_chroms.tolist(), reverse=True)

    # # # --- Differential Evolution
    # mat_chrom_accur = do_diff_chrom_v3(np.asarray(mat_chroms), meta['num_differential_sol'], num_chrom_params)
    # mat_chroms = sorted(mat_chrom_accur, reverse=True)
